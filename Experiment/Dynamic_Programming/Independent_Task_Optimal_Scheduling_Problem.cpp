...
void TraceBack(int N, int at)
{
    ...
    for (int i = N; i >= 1; i--)
    {
        // 首先假设该工件在 A上加工。如果不加工这个工件时 B消耗的时间与加工该工件时 B消耗的时间相等，说明假设可以成立，即最后一个工件是在 A机器上加工的
        ...
        else // 这说明最后一个工件是在 B机器上加工的（不是在 A上就是在 B上）
        {
            x[i] = 2;
            sumt_b -= b[i];
        }
...
/*
 ...
 选择花费时间小的方式。
 在回溯寻找最优解时，从找到最优时间的位置开始，先判断最后一个工件（未判断过的）是否在 A机器上加工。如果不是，则是在 B机器上加工。
*/
